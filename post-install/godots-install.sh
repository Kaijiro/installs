...
# Create modular shell config for Godots, same pattern as nvm/sdkman/pyenv:
# sourced automatically via .zshrc's ~/.dotfiles/*.zsh loop, so godots is on
# PATH without the user having to hand-edit a shell rc file.
DOTFILES_DIR="$HOME/.dotfiles"
mkdir -p "$DOTFILES_DIR"

GODOTS_CONFIG="$DOTFILES_DIR/godots.zsh"
cat > "$GODOTS_CONFIG" <<'EOF'
# ============================================================
# Godots (Godot Version Manager) Configuration
# ============================================================
# Auto-generated by godots-install.sh

export PATH="$HOME/.local/bin:$PATH"
EOF

echo "✅ Created modular shell config: $GODOTS_CONFIG"
echo ""
echo "Usage:"
